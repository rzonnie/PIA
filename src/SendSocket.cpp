...
void SendSocket::sendPacket(char packet[]) {
    /**
    ...
    std::cout << "Packet: " << strlen(packet) << std::endl;

    //send a packet every 5 seconds
    if (sendto(sock, packet, strlen(packet), 0, (struct sockaddr*) &multicastSender, sizeof (struct sockaddr_in)) < 0) //sent a UDP packet containing our example data
